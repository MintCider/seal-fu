...
> [!WARNING]  
> `v1.0.0` 依据官方中文版规则书更新了部分术语翻译（感知→洞察、状态效果→异常状态、先攻修改值→先攻修正值），导致人物卡的数据存储格式改变。从旧版本升级而来的话，会出现数据不匹配。可以通过 `.st clr` 清空旧人物卡属性，参考最新术语翻译，重新创建人物卡。
...
> [!NOTE]
> 本插件中的术语翻译已基于官方中文版规则书（2026.02.06 版）进行更新。
...
> [!NOTE]
> 设定敏捷、洞察、力量、意志四维属性时，请输入对应属性骰的面数（6、8、10、12 之一）。当属性骰发生变动时（如异常状态、临时 buff 等），请不要使用 `.st` 修改，请参考后面的 `.buff` 和 `.ds` 命令。

以下几个例子都是合理的属性设定方式：

```text
.st 卡珊德拉-敏捷10洞察6力量8意志8生命值50生命值上限50精神值50精神值上限50物资点6物资点上限6物语点3金币170先攻-2物防11魔防8
```
...
> [!NOTE]
> 从 `v0.3.0` 版本开始，插件支持物防值、魔防值与属性骰联动。当属性骰由于异常状态或临时 buff 发生变动时，物防值、魔防值会自动进行调整。
>
...
此命令支持的属性为：敏捷、洞察、力量、意志，以及任何对应四维属性的属性别名（例如：dex、ins、mig、wlp），不区分大小写。属性别名参见上文表格，如需修改属性别名，请参考后文的配置项。

属性后可以直接跟数字指定骰面数，适用于未录入角色卡的 NPC 等情况（例如：`mig10+mig10`）。
...
```text
<卡珊德拉>的敏捷+洞察+1检定结果为：d8（迟缓）+d6+1=[4+6+1]=11
HR：6
```

如果由于异常状态或临时 buff 导致属性骰发生变动，检定回复中会进行提示。如果检定出大成功/大失败，检定回复会进行提示。同时，检定回复会自动显示 HR。
...
```text
<卡珊德拉>的先攻检定结果为：d8（迟缓）+d6+0（先攻修正）+3=[7+3+0+3]=13
```

### `.buff` 异常状态

此命令支持的异常状态为：迟缓、眩晕、虚弱、动摇、激怒、中毒。
...
> [!NOTE]
> 此命令与异常状态产生的影响叠加计算，并且永远不会超出属性骰的上下限。
...
如上文所述，属性骰发生变动时（如异常状态、临时 buff 等），不应使用 `.st` 修改，而应该使用 `.buff` 或 `.ds` 命令。但如果出于游戏需要，使用 `.st` 命令修改了属性骰的原始值，则需要使用 `.eval` 命令，根据异常状态、临时 buff 等，重新核算属性。
